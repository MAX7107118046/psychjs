...
// HTML DOM styles (by using JS function alone or in jsPsych 'on_start', 'on_load', 'on_finish' parameters)HTML DOM样式(通过单独使用JS函数或在jsPsych中使用'on_start'， 'on_load'， 'on_finish'参数)
...
const subID = jsPsych.randomization.randomID(4)
...
var time1 = ("当前时间是："+hour+"时"+minute+"分"+second+"秒");
...
var iat_temp = {
    ...
    attribA: { label: '女性', items: ['蓝语薇', '罗素梅', '张莹莹', '何惠琴', '赵晓倩'] },
    attribB: { label: '男性', items: ['王一凯', '林志建', '祝家杰', '李政祥', '李德忠'] },
    targetA: { label: '男性食物', items: ['啤酒', '韭菜', '可乐', '牛肉', '羊肉'] },
    targetB: { label: '女性食物', items: ['奶茶', '蛋糕', '冰淇淋', '甜品', '草莓'] },
}
...
// Top-left and top-right tags左上角和右上角标签
...
var IAT_instr1 = {
    ...
    —— 任务1：对“${iat.attribA.label}”词和“${iat.attribB.label}”词反应 ——<br/>
...
var IAT_instr2 = {
    ...
    stimulus: `
    <div class="tag-bottom"><p>
    —— 任务2：对“${iat.targetA.label}”词和“${iat.targetB.label}”词分反应 ——<br/>
...
var IAT_instr4 = {
    ...
    stimulus: `
    <div class="tag-bottom"><p>
    —— 任务4：对“${iat.attribA.label}/${iat.targetA.label}”词和“${iat.attribB.label}/${iat.targetB.label}”词反应 ——<br/>
    <span style="color:#78DCE8"><b>与刚才的任务完全相同，请再次对这四类词语反应</b></span><br/>
    <span style="color:#FFD866"><b>请根据上方标签的提示，尽可能正确并且快速地做出按键反应</b></span><br/>
    当按键错误时屏幕中会出现<span style="color:red"> X </span>，需要按另一个键纠正才能继续<br/><br/>
    请把双手食指分别放在键盘的“${key_L.toUpperCase()}”键和“${key_R.toUpperCase()}”键上<br/>
    按<空格键>开始
...
var IAT_instr7 = {
    ...
    <span style="color:#FF6188"><b>与刚才的任务完全相同，请再次对这四类词语反应</b></span><br/>
    <span style="color:#FFD866"><b>请根据上方标签的提示，尽可能正确并且快速地做出按键反应</b></span><br/>
    当按键错误时屏幕中会出现<span style="color:red"> X </span>，需要按另一个键纠正才能继续<br/><br/>
    请把双手食指分别放在键盘的“${key_L.toUpperCase()}”键和“${key_R.toUpperCase()}”键上<br/>
    按<空格键>开始
...
function blockTemplateIAT(block_id, tag, stimuli, stim_func, key_answer_func, iti = 400) {
    var IAT = {
        ...
        timeline: [{
            ...
            prompt: tag,
            trial_duration: 100, // inter-trial interva。linter-trial间隔
            ...
            force_correct_button_press: false,//锁定答案
...
function replaceWrongRT(df) {
	//用块均值+ 600 ms替换每个错误试验RT (grewald et al.， 2003, JPSP)
    var rt_mean = df.filter({ correct: true }).select('rt').mean()
    var wrong = df.filter({ correct: false }).values() // 原始数据阵列(可修改)
    for (var i in wrong) {
...
var debrief_IAT = {
    ...
    stimulus: function() {
		...
		//参见grewald等人(2003)中IAT D-score的评分算法。
        if (compatible_first) {
    ...
    on_finish: function(data) {
        ...
        data.summary = JSON.stringify(IAT_results)
			...
			//提取R: jsonlite::fromJSON(子集(数据，varname=='IAT_feedback')$summary)
    }
}
